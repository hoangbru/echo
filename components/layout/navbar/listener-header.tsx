...
import { SearchInput } from "@/components/features/listener/search";
...
interface ListenerHeaderProps {
  onOpenSidebar: () => void; // Thêm prop này
}
...
export function ListenerHeader({ onOpenSidebar }: ListenerHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 w-full bg-background/80 backdrop-blur border-b border-border">
      <div className="flex items-center justify-between px-6 py-4 gap-4 md:gap-6">
        <button
          onClick={onOpenSidebar}
          className="lg:hidden text-muted-foreground hover:text-foreground transition-colors outline-none flex-shrink-0"
          aria-label="Open sidebar"
        >
          <Menu className="h-6 w-6" />
        </button>

        <div className="flex-1 md:flex-initial w-full max-w-[360px]">
          <SearchInput />
        </div>
